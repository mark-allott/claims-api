...
namespace ClaimsApi.Data.Interfaces
{
	public interface IQueryRepository<TEntity> where TEntity : class
	{
		...
		/// <param name="skipCount">The number of entities to skip over when returning results (0 denotes start from beginning)</param>
		/// <returns>An <see cref="IEnumerable{T}"/> of results matching the requirements</returns>
		IEnumerable<TEntity> All<TKey>(Expression<Func<TEntity, TKey>>? orderByPredicate = null,
		...
		Task<TEntity?> FirstOrDefaultAsync(Expression<Func<TEntity, bool>> filterPredicate,
		...
		Task<TEntity?> SingleOrDefaultAsync(Expression<Func<TEntity, bool>> filterPredicate,
			CancellationToken token = default);
